...
class EmployeControllerIntegrationTests {
	...
	private Employe employe = new Employe("mohamed", "ali", "m.ali@esprit.tn", true, Role.INGENIEUR);
	...
	void testMettreAjourEmailByEmployeId() throws JsonProcessingException, Exception {
		String newEmail = "mail@gmail.com";
	...
	@DisplayName("ajouterEmploye test")
	@Test
	void testAjouterEmploye() throws JsonProcessingException, Exception {
		String emloyeeJSON = new ObjectMapper().writeValueAsString(employe);
		MvcResult mvcResult = this.mockMvc
				.perform(post("/ajouterEmployer").contentType("application/json").content(emloyeeJSON)).andDo(print())
				.andExpect(status().isOk()).andReturn();
		employe.setId(employe.getId() + 1);
		emloyeeJSON = new ObjectMapper().writeValueAsString(employe);
